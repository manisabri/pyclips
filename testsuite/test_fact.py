...
class Fact(CTestCase):
    ...
    def test_Template_05(self):
        ...
        for x in list(self.envdict.keys()):
            ...
            self.assertTrue(not t1.Slots.AllowedValues("s1"))
    ...
    def test_Template_07(self):
        ...
        for x in list(self.envdict.keys()):
            ...
            t1 = e.BuildTemplate(
                ...
                (slot s2 (range 13 42))
                """, "test template")
            self.assertTrue(t1.Slots.Exists("s0"))
            self.assertTrue(t1.Slots.Exists("s1"))
            self.assertTrue(t1.Slots.Exists("s2"))
            self.assertTrue(not t1.Slots.Exists("s3"))
            rs0 = t1.Slots.Range("s0")
            self.assertEqual(rs0[0], '-oo')
            self.assertEqual(rs0[1], '+oo')
            self.assertTrue(not t1.Slots.Range("s1"))
    ...
    def test_Template_08(self):
        ...
        for x in list(self.envdict.keys()):
            ...
            self.assertTrue('s0' in n)
            self.assertTrue('s1' in n)
            self.assertTrue('s2' in n)
            self.assertTrue('s3' not in n)
            t = list(map(str, t1.Slots.Types("s1")))
            self.assertTrue('STRING' in t)
            self.assertTrue('SYMBOL' in t)
            self.assertTrue('INTEGER' not in t)
    ...
    def test_Template_09(self):
        ...
        for x in list(self.envdict.keys()):
            ...
            t1 = e.BuildTemplate(
                ...
                (multislot s2)
                """, "test template")
            self.assertTrue(t1.Slots.IsSinglefield('s1'))
            self.assertTrue(not t1.Slots.IsMultifield('s1'))
